...
const onRrefreshed = (newToken: string | null) => { // 파라미터 타입 변경 (string | null)
    refreshSubscribers.forEach((callback) => callback(newToken));
    refreshSubscribers = [];
};

const addSubscriber = (callback: (newToken: string | null) => void) => { // 파라미터 타입 변경 (string | null)
...
JwtAxios.interceptors.response.use(
    ...
    async (error: AxiosError): Promise<AxiosResponse | AxiosError | null> => { // 반환 타입 명시 (AxiosResponse | AxiosError | null)
        ...
        if (error.response && (error.response.status === 401 || error.response.status === 403)) {
            if (!isRefreshing) {
                ...
                } catch (refreshError) { // catch 블록에서 에러 변수명 변경 (refreshError)
            ...
            return new Promise((resolve) => {
                addSubscriber((newToken) => {
                    if (originalRequest && newToken) { // newToken null 체크 추가
                        originalRequest.headers = {
                            ...originalRequest.headers,
                            Authorization: `Bearer ${newToken}`
                        } as Record<string, string>;
